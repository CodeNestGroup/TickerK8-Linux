...
def NewsReadReloadStyle(self):
    m = open(f'{self.Path}/assets/CSS/NewsReadMain.css').read()
    c = open(f'{self.Path}/assets/CSS/NewsRead{self.Theme}.css').read()
    self.setStyleSheet(m+c)

def NewsReadRetranslate(self):
    t = json.load(open(f'{self.Path}/assets/JSON/NewsReadTranslate.json', 'r', encoding='utf-8'))
    l = self.Language
    ItemName = ''
    if self.NewsData[4]:
